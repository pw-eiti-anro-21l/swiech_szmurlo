...
def get_parameters(part):
    with open("params_server.json", "r") as read_file:
...
if __name__ == "__main__":
    ...
    base_urdf = create_xml_link(0, 0, 0, 0, 0, 0, 1, "base")
    tool_urdf = create_xml_link(0, 0, 0, 0, 0, 0, 0.2, "tool")
    body_tool_joint_urdf = create_xml_joint(0, 0, 0, 0, 0, 0, "joint_tool")

    tree = ET.Element("robot", name = "my_robot")
    tree.extend([base_urdf])
    tree.extend(array_of_urdfs)
    tree.extend([body_tool_joint_urdf])
    tree.extend([tool_urdf])
    ET.ElementTree(tree).write('my_robot.urdf.xml', pretty_print=True)
    print("URDF generated to my_robot.urdf.xml")

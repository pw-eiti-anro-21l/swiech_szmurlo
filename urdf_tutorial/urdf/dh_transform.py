...
def roll_pitch_yaw_params(rotation_matrix):
    # tan_phi = (rotation_matrix[2][1])/(rotation_matrix[2][2])
    # tan_theta = (-1*rotation_matrix[2][0]/(sqrt((rotation_matrix[2][1])**2 + (rotation_matrix[2][2])**2)))
    # tan_psi = (rotation_matrix[1][0]/rotation_matrix[0][0])
    # phi = atan(tan_phi)
...
def create_yaml_dict(r, p, y):
    dict_file = [{'params' : [{'roll': r}, {'pitch': p}, {'yaw': y}]}]
    return dict_file
...
if __name__ == "__main__":
    ...
    with open ("/home/maciej/dev_ws/src/swiech_szmurlo/urdf_tutorial/urdf/rpy.yaml", "w") as yaml_file:
